...
class SlugElement {
    ...
    private registerEvents(): void {
        ...
        // sluggi
        this.readOnlyField.addEventListener('focus', () => {
            this.readOnlyField.blur();
        });
    ...
    private sendSlugProposal(mode: ProposalModes): void {
        ...
        this.request.post({
            ...
            let visualProposal = '/' + data.proposal.replace(/^\//, '');
            ...
            // sluggi
            if (data.inaccessibleSegments) {
                data.proposal = data.proposal.substring(data.inaccessibleSegments.length);
            }
            if (data.lastSegmentOnly) {
                data.proposal = data.proposal.substring(data.proposal.lastIndexOf('/'));
            }
